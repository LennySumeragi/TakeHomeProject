...
if "cluster" not in df.columns:
    raise ValueError("clusters.csv must contain a 'cluster' column")

# ---------------- Compute Top Features ----------------
clusters = sorted(df["cluster"].unique())

# Compute overall mean (to compare deviations)
overall_mean = df.drop(columns=["cluster"]).mean(numeric_only=True)

for c in clusters:
    cluster_df = df[df["cluster"] == c].drop(columns=["cluster"])
    cluster_mean = cluster_df.mean(numeric_only=True)

    # Absolute deviation from overall mean
    diff = (cluster_mean - overall_mean).abs()

    # Top 5 features for this cluster
    top_feats = diff.sort_values(ascending=False).head(5)

    # Print results
    print(f"\nCluster {c} Top Contributing Features:")
    print(top_feats)

    # Plot bar chart
    plt.figure(figsize=(6,4))
    sns.barplot(x=top_feats.values, y=top_feats.index, palette="viridis")
    plt.title(f"Cluster {c} - Top 5 Contributing Features")
